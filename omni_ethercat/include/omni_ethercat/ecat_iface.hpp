...
public:
	...
	unsigned int w_target_position;
	unsigned int w_target_velocity;
	unsigned int w_target_torque;
	unsigned int w_max_torque;
	unsigned int w_controlword;
	unsigned int w_mode_of_operation;
	unsigned int w_profile_velocity;
	unsigned int w_profile_acceleration;
	unsigned int w_profile_deceleration;
...
public:
	EcatAdmin();
	int ecat_init();  //Should be the first thing to be called after instantiating. Check the return value to decide what to do afterwards.
	bool finished_ecat_init();  // Will return true if the ecat_init has finished happily

	void check_drive_state();   //should be called periodically to check on the state of the drives (and reactivates them if they went down)
	void shutdown();
...
private:
	...
	static void * realtimeMainEntryFunc(void * This) {((EcatAdmin *)This)->realtime_main(); return NULL;}
