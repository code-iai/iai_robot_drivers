...
void print_pdo_entry_reg(ec_pdo_entry_reg_t reg) {
	std::cout << "{" << reg.alias << ", " << reg.position << std::hex << " ,0x" << reg.vendor_id
			<< ", 0x" << reg.product_code << ", 0x" << reg.index << ", 0x" << static_cast<uint16_t>(reg.subindex) << ", 0x"
...
void EcatAdmin::realtime_main()
{
	...
	while(!rt_should_exit)
	{
		...
		if ((now.tv_sec + static_cast<double>(now.tv_nsec)/1e9) > (tick.tv_sec + static_cast<double>(tick.tv_nsec)/1e9))
		{
...
int EcatAdmin::ecat_init(){
	...
	if (init_took_too_long) {
		std::cerr << "ecat_init(): it took too long for the bus to come up (ec_domain_running). Aborting the init." << std::endl;
...
void EcatELMODrive::setup_variables() {
	...
	add_pdo_entry(0x607A, 0x0, &pdata_offsets.w_target_position, 0);
	add_pdo_entry(0x60FF, 0x0, &pdata_offsets.w_target_velocity, 0);
	add_pdo_entry(0x6071, 0x0, &pdata_offsets.w_target_torque, 0);
	add_pdo_entry(0x6072, 0x0, &pdata_offsets.w_max_torque, 0);
	add_pdo_entry(0x6040, 0x0, &pdata_offsets.w_controlword, 0);
	add_pdo_entry(0x6060, 0x0, &pdata_offsets.w_mode_of_operation, 0);
	add_pdo_entry(0x6081, 0x0, &pdata_offsets.w_profile_velocity, 0);
	add_pdo_entry(0x6083, 0x0, &pdata_offsets.w_profile_acceleration, 0);
	add_pdo_entry(0x6084, 0x0, &pdata_offsets.w_profile_deceleration, 0);
...
void EcatELMODrive::task_data_to_process() {
	EC_WRITE_S32(domain_pd_ + pdata_offsets.w_target_position, task_wdata_process_side.target_position);
	EC_WRITE_S32(domain_pd_ + pdata_offsets.w_target_velocity, task_wdata_process_side.target_velocity);
	EC_WRITE_S16(domain_pd_ + pdata_offsets.w_target_torque, task_wdata_process_side.target_torque);
	EC_WRITE_S16(domain_pd_ + pdata_offsets.w_max_torque, task_wdata_process_side.max_torque);
	...
	EC_WRITE_U32(domain_pd_ + pdata_offsets.w_profile_velocity, task_wdata_process_side.profile_velocity);
	EC_WRITE_U32(domain_pd_ + pdata_offsets.w_profile_acceleration, task_wdata_process_side.profile_acceleration);
	EC_WRITE_U32(domain_pd_ + pdata_offsets.w_profile_deceleration, task_wdata_process_side.profile_deceleration);
